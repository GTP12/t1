...
;(function($){
	...
	var checkbox = {
		...
		toggle: function(e) {
			if(!$(this).parent().hasClass("nochose")){
				$(this).toggleClass('checkbox_active').children().attr('checked', ($(this).hasClass('checkbox_active') ? true : false));
			}
		},
		allOrNone: function(e) {
			...
			if(cbAllName.length > 3){
				...
				var isChecked = $(this).hasClass('checkbox_active')? true : false;
				if(isChecked){
					$('.checkbox[name='+ cbOneName +']').not($('.checkbox_active[name='+ cbOneName +']')).click();
				}else{
					$('.checkbox_active[name='+ cbOneName +']').click();
				}
			}
		}
	};
